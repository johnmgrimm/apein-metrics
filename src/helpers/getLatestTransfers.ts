...
export async function getLatestTransfers(
  ...
  const match = getMatch(dateString, direction);
  const response = await apiFetch<CovalentApiResponse>(
    `https://api.covalenthq.com/v1/${chainId}/address/${address}/transfers_v2/?contract-address=${contractId}&key=${covalentApiKey}&match=${match}&group=${group}`,
  );
